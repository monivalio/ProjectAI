...
public class ReversiCut {
	...
	public static int[] directions = new int[] { LEFT, RIGHT, UP, DOWN,
	...
	public static Position GetNextMove(GameState gameState,
		...
		for (int i = 0; i < child.length; i++) {
			h = gameStage == GameStage.StartGameStage ? GameStateHeuristics
					.basicHeuristics(child[i].board, 2) : next(child[i],
					depth - 1, 1);
			if (h > max) {
				max = h;
	...
	public static int next(GameState gameState, int depth, int player) {
		if (depth == 0) {
			return GameStateHeuristics.endGameHeuristics(gameState.board,
	...
	public static GameState createGameBoard(String gameBoardStr) {
		...
		int xPos = 0;
		int yPos = 0;
		int inputLength = gameBoardStr.length();
		for (int i = 0; i < inputLength; i++) {
			...
			} else 	if (gameBoardStr.charAt(i) == '0' || gameBoardStr.charAt(i) == '*') {
			...
			} else if (gameBoardStr.charAt(i) == 'G') {
				gameBoard[xPos][yPos] = -1;
				yPos++;
				if (yPos == 8) {
					xPos++;
	...
	public static int[][] flipDiscs(int[][] gameBoard, Position pos,
			int flipColor) {
	...
	private static void flipInDirection(int[][] gameBoard, int direction,
			Position pos, int flipColor) {
		if (direction == LEFT) {
	...
	public static boolean canFlipInDirection(int[][] gameBoard, int direction,
			Position pos, int flipColor) {
		if (direction == LEFT) {
			for (int j = pos.getY() - 1; j >= 0; j--) {
	...
	public static GameState[] getReachableGameStates(GameState gameState,
			int player) {
		...
		for (Position pos : validPositions) {
			...
			childGameState.validPositions = getValidPositions(
					childGameState.board, player == 2 ? 1 : 2);
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (board[i][j] == 0) {
					...
					for (int k = 0; k < 8; k++) {
						if (canFlipInDirection(board, directions[k], pos,
								player)) {
							isValid = true;
							break;
						}
	...
	public static boolean allDiscsFlippedInDirection(int[][] board,
			int direction, int x, int y, int player) {
		if (direction == LEFT) {
			for (int i = y - 1; i >= 0; i--) {
				if (board[x][i] != player) {
					return false;
